...
from tools.search_articles import SearchAndContents, FindSimilar, GetContents
...
class NewsletterGenCrew:
    ...
    def newsletter_task(self) -> Task:
        return Task(
            ...
            agent=self.designer(),
            output_file=f"news/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_{str(uuid4())}.html",
        )
    ...
    def edit_task(self) -> Task:
        return Task(
            ...
            agent=self.editor(),
            output_file=f"newspapers/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_edit_task.md",
        )
